...
public class SaveMigrator {
    ...
    public static void migrateFromYml(CommandSender sender) {
        Bukkit.getScheduler().runTaskAsynchronously(SimpleNicks.getInstance(), () -> {
            ...
            try {
                ...
                sender.sendRichMessage(LocaleMessage.ERROR_UNABLE_TO_LOAD_FILE.getMessage());
                return;
            }
            int migratedUsers = 0;
            for (String uuidString : nicknameData.getKeys(false)) {
                ...
                try {
                    ...
                    debug("Skipping invalid UUID key: {}", uuidString);
                    continue;
                }
                ...
                debug("Migrating nicknames for user: {} - (UUID:{})", username, uuidString);
                if (currentNick != null && !currentNick.isEmpty()) {
                    if (Cache.getInstance().getActiveNickname(player.getUniqueId()) == null) {
                        boolean saved = Cache.getInstance().setActiveNickname(uuid, username, currentNick);
                        if (saved) {
                            debug("Active nickname: {}", currentNick);
                        } else {
                            debug("Failed to save current nickname: {}", currentNick);
                        }
                    }
                }
            ...
            sender.sendRichMessage(LocaleMessage.MIGRATION_FINISHED.getMessage());
