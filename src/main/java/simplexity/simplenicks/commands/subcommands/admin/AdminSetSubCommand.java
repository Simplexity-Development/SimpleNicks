...
public class AdminSetSubCommand implements SubCommand {
    ...
    public int execute(@NotNull CommandContext<CommandSourceStack> ctx) throws CommandSyntaxException {
        ...
        String nickname = NickUtils.getInstance().cleanNonPermittedTags(sender, inputNickname.nickname());
        //todo idk my brain is broken, this needs straightening up tho
        Nickname cleanedNickname = new Nickname(nickname, SimpleNicks.getMiniMessage().stripTags(nickname));
        NickUtils.getInstance().nicknameChecks(sender, cleanedNickname);
        ...
        sender.sendMessage(parseAdminMessage(Message.CHANGED_OTHER.getMessage(), nickname, sender, target));
        if (target instanceof Player onlineTarget) onlineTarget.sendMessage(parseAdminMessage(Message.CHANGED_BY_OTHER.getMessage(), nickname, sender, target));
    ...
    public boolean canExecute(@NotNull CommandSourceStack css) {
        ...
        return sender.hasPermission(Constants.NICK_SET_OTHERS);
    }
