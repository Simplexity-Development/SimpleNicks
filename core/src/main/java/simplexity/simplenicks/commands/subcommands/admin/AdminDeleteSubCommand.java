...
public class AdminDeleteSubCommand<S> implements SubCommand<S> {

    private final BrigadierAdapter<S> adapter;

    public AdminDeleteSubCommand(@NotNull BrigadierAdapter<S> adapter) {
    ...
    public void subcommandTo(@NotNull LiteralArgumentBuilder<S> parent) {
        parent.then(adapter.literal("delete").requires(this::canExecute)
                .then(adapter.playerThenTargetNicknameArgument("player", "nickname", this::execute)));
    }
                ...
                SimpleNicksCore.get().platform().runSync(() -> {
                    if (SimpleNicksCore.get().platform().isPlayerOnline(target.id())) {
                        ...
                        SimpleNicksCore.get().platform().sendMessageToPlayer(target.id(),
                                parseAdminMessage(LocaleMessage.DELETED_BY_INITIATOR.getMessage(),
                                        nickname.getNickname(), senderCtx, targetName));
                    }
                    senderCtx.sendMessage(parseAdminMessage(LocaleMessage.DELETED_TARGET.getMessage(),
                            nickname.getNickname(), senderCtx, targetName));
                });
            } else {
                sendFeedback(senderCtx, LocaleMessage.ERROR_DELETE_FAILURE, nickname);
            }
        });
    ...
    public boolean canExecute(@NotNull S source) {
        return adapter.hasPermission(source, NickPermission.NICK_ADMIN_DELETE);
    }
}
